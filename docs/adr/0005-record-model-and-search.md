...
The generic CSV path accepts UTF-8 input with or without a BOM, bounded to 16 MiB and 10,000 data rows. It detects comma, semicolon, tab, or pipe delimiters when unambiguous and otherwise requires an explicit choice. Before a final preview, the user selects Login, Secure Note, or Generic Item and must explicitly map or ignore every header column. Invalid records are reported by row and prevent confirmation; CSV quoting, embedded newlines, and Unicode remain the responsibility of the local parser.

Portable backups serialize the complete encrypted cache and its reconciliation metadata inside a versioned, authenticated envelope. Argon2id derives a backup-specific XChaCha20-Poly1305 key from a separate backup password; the header exposes no semantic Vault data. Restore decrypts and validates the complete payload before preview, preserves the complete graph for an empty same-account cache, or assigns fresh deterministic Item IDs and remaps Folder associations for semantic restores. Every native field and conflict head is retained, and idempotent offline-first mutations are queued only after explicit confirmation.
